...
export default function Layout({ user, onLogout, children }) {
  ...
  return (
    <div className="flex h-screen bg-slate-950 text-slate-300 relative overflow-hidden font-sans">
      {/* Ambient Glow */}
      ...
      {/* Mobile Header */}
      <header className="md:hidden fixed top-0 left-0 right-0 h-16 bg-slate-900/80 backdrop-blur-xl border-b border-white/5 z-40 flex items-center justify-between px-5 shadow-lg">
      ...
      {/* Sidebar Navigation */}
      <aside className={`fixed inset-y-0 left-0 z-50 w-72 bg-slate-900/90 backdrop-blur-xl border-r border-white/5 flex flex-col transition-transform duration-300 ease-out md:relative md:translate-x-0 shadow-2xl md:shadow-none ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
        ...
        <nav className="flex flex-col gap-2 flex-1 px-4 mt-6 md:mt-0">
          {navItems.map(item => (
            <Link
              ...
              className={`px-4 py-3.5 rounded-xl text-sm font-semibold transition-all duration-200 flex items-center gap-3 ${pathname === item.path ? 'bg-emerald-500/15 text-emerald-400 border border-emerald-500/20 shadow-[inset_0px_1px_1px_rgba(255,255,255,0.05)]' : 'text-slate-400 hover:text-white hover:bg-white/5'}`}
            >
              <item.icon />
              {item.label}
            </Link>
