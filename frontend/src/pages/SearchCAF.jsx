...
export default function SearchCAF() {
  ...
  const startListening = () => {
    ...
    recognition.onerror = (event) => { if(event.error !== 'no-speech') toast.error('Voice search failed.'); };
      ...
      <div className="sticky top-0 z-40 bg-[var(--bg-base)] pb-4 -mt-4 pt-2 -mx-4 px-4 md:-mt-8 md:pt-8 md:-mx-8 md:px-8 border-b border-[var(--border-color)] mb-4 shadow-sm md:shadow-none">
        ...
        <div className="fade-up stagger-1 relative max-w-2xl mx-auto">
          ...
          <div className="absolute inset-y-0 right-2 md:right-3 flex items-center">
            {query ? (
               <button onClick={() => setQuery('')} className="p-2 text-slate-400 hover:text-slate-600 dark:hover:text-slate-200 transition-colors rounded-full hover:bg-[var(--border-color)]">
                  <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2.5"><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
               </button>
            ) : (
              <button onClick={startListening} className={`p-2 rounded-full transition-colors ${isListening ? 'bg-red-500 text-white animate-pulse shadow-md shadow-red-500/40' : 'text-emerald-500 hover:bg-emerald-500/10'}`}>
